...
def main():
    # 購入者を取得
    buyers = file_util.read_csv('buyer.csv')
    # 商品を取得
    products = file_util.read_csv('product.csv')
    # 購入優先度リストを取得
    wishlists = file_util.read_csv('wishlist.csv') 
    ...
    for buyer in buyers:
        ...
        buyer_model = Buyer(id=buyer['id'], name=buyer['name'], wishlist=wishlist)
        buyer_models.append(buyer_model)

    # 商品
    product_models: list[Product] = [Product(**product) for product in products]
    ...
    for buyer in buyer_models:
        ...
        for product_name, num_of_buy in zip(wishlist.priority, wishlist.number_of_buy):
            ...
            if not validate_product(product_name, product_models):
                log.warning(f'選択した商品は存在しません。 購入者={buyer.name}, 商品名={product_name}')
                continue
                
            # 購入数が多いものは弾く
            if not validate_buy_of_num(num_of_buy):
                log.warning(f'商品2つまでしか購入できません。購入者={buyer.name}, 購入数={num_of_buy}')
                continue
    ...
    for key in drawings:
        ...
        if len(drawings[key]) == 0:
            ...
            continue
        max_number = max(drawings[key], key=lambda x: x['number_of_buy'])['number_of_buy']
        min_number = min(drawings[key], key=lambda x: x['number_of_buy'])['number_of_buy']
        buyers_of_product = drawings[key]
        # 最大値と最小値が混在している場合は最大値をリストから除去する
        if max_number != min_number:
            buyers_of_product = [buyer for buyer in drawings[key] if buyer['number_of_buy'] == min_number]
        ...
        # 対象者が複数人いる場合は、ランダム選出
        random_buyers = random.sample(buyers_of_product, len(buyers_of_product))
        # 2回抽選
        drawing_result[key] = random_buyers[random.randrange(len(random_buyers))]
